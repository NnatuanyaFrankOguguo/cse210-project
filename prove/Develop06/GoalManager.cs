    ...
    public void RecordEvent()
{
    ...
    if (int.TryParse(Console.ReadLine(), out int goalIndex) && goalIndex >= 1 && goalIndex <= _goals.Count)
    {
        ...
        // If the goal is a checklist goal, add bonus points if fully completed
        if (selectedGoal is ChecklistGoal checklistGoal)
        {
            if (checklistGoal.IsComplete())
            {
                ...
                _score += bonusPoints;
                Console.WriteLine(" ");
                Console.WriteLine($"Congratulations! You've completed the checklist goal and earned an additional {bonusPoints} bonus points!");
            }
